...
async function translateText(text, targetLang) {
      ...
      if (error.code === 'ECONNABORTED' || (error.response && error.response.status === 429)) {
        try {
          ...
          if (fallbackResponse.data && fallbackResponse.data.translation) {
            return fallbackResponse.data.translation;
          }
        } catch (fallbackError) {
  ...
  try {
    ...
    // If it returned the original text due to an error, we might want to drop it from cache
    // so subsequent requests can try again, but for now we keep it to prevent spamming failing APIs.
    return translated;
  } catch (err) {
